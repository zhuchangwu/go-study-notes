...
自定义类型指的是我们可以通过Go原生为我们指定的数据类型如 string，整形，浮点型，布尔等数据类型定义我们自己的数据类型如下：
...
我们对一个结构体变量进行取地址& , 就会得到这个结构体变量的指针
